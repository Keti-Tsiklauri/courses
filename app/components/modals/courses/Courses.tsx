...
export default function Courses() {
  ...
  // Fixed container height for scroll even if no courses
  const containerHeight = "min-h-[1200px]"; // adjust height as needed

  return (
    <div className={`w-[1170px] ${containerHeight} overflow-y-auto`}>
      {filteredCourses.length === 0 ? (
        <div className="flex justify-center items-center h-full mt-[100px]">
          <p className="text-gray-500 text-lg font-medium">
            Sorry, there are no courses
          </p>
        </div>
      ) : (
